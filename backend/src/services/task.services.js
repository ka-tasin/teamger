...
export class TaskService {
    ...
    async reassignTasks(teamId, userId) {
    ...
    async getTeamMemberSummary(teamId) {
        const teamMembers = await this.teamMemberRepository.findByTeamId(teamId);
        const tasks = await this.taskRepository.findByTeamId(teamId);
        return teamMembers.map(member => {
            const currentTasks = tasks.filter(task => task.assignedTo === member.id && task.status !== 'Done').length;
            return {
                memberId: member.id,
